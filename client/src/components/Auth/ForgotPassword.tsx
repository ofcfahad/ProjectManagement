    ...
    return (
        <motion.div className='flex flex-col bg-white/30 backdrop-blur h-[200px] px-3 rounded-3xl'>
            <div className='flex items-center mt-3'>
                <ArrowLeftIcon className='w-5 text-black cursor-pointer' onClick={handleCloseDialogClick} />
            ...
            <form onSubmit={submitFunction} className='mt-4'>
                <motion.div initial={{ borderRadius: 0 }} animate={{ borderRadius: userEmailInputController != '' || inputIsFocused === 'userEmailInputBar' ? 0 : 10 }} className={`flex items-center px-2 border-2 border-transparent rounded-none ${userEmailInputController != '' ? 'border-b-blue-600' : inputIsFocused === 'userEmailInputBar' ? 'bg-transparent border-b-selectedicon' : 'bg-white rounded-[8px]'}`}>
                    ...
                    <div className='ml-2'>
                        <input type="text" className={`px-2 py-2 bg-transparent focus:outline-none rounded-xl autofill:bg-black `} id='userEmailInputField' placeholder='Email' autoComplete='email' value={userEmailInputController} onChange={(event) => setUserEmailInputController(event?.target?.value ?? '')} onFocus={() => setInputIsFocused('userEmailInputBar')} onBlur={() => setInputIsFocused('')} />
                    </div>
                </motion.div>
                ...
                <div className='w-full flex justify-center items-center mt-4'>
                    ...
                    } title={'Email Sent'} isOpen={emailSent} onClose={handleCloseDialogClick} description={`If you're registered with, an Email is sent to ${userEmailInputController}. Click the link provided in the Email and set a new Password. The link in the Email will expire in 1 hour `} />
                </div>
            </form>
        </motion.div>
    )
